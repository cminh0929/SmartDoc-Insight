...
- Mặc dù dùng chung Database, dữ liệu của các doanh nghiệp vẫn được cô lập hoàn toàn ở mức đ�1. **Khóa ngoại bắt buộc:** Mọi thực thể nghiệp vụ (Users, Roles, Folders, Documents, DocumentChunks, AuditLogs) đều bắt buộc phải chứa cột `tenantId` (kiểu UUID). Nếu truy vấn thêm mới dữ liệu (INSERT) mà thiếu trường này, Drizzle ORM sẽ lập tức báo lỗi.
...
3. **Tuyệt đối an toàn:** Dữ liệu của Công ty A không bao giờ có thể bị truy cập bởi người dùng của Công ty B, ngay cả khi người dùng B đoán trúng chính xác ID (UUID) của tài liệu hay phân đoạn vector. API sẽ luôn luôn trả về lỗi `404 Not Found`.
...
Để dễ hình dung cấu trúc lưu trữ, dưới đây là **Sơ đồ Thực thể Liên kết (ERD)** minh họa cách các bảng quan hệ với nhau xoay quanh trục lõi là `Tenants` bao gồm cả các thực thể Vector RAG mới:
...
erDiagram
    ...
    TENANTS ||--o{ DOCUMENT_CHUNKS : "isolates vector chunks"
    ...
    DOCUMENTS ||--o{ DOCUMENT_CHUNKS : "vectorized into"
...
- **Bảng `DocumentChunks`:** Chứa các phân mảnh văn bản của tài liệu đã được trích xuất và số hóa thành các Vector Embedding 1536 chiều. Bảng này liên kết chặt chẽ `1-N` với `Documents` và bắt buộc phải cô lập theo `tenantId` để ngăn chặn chéo quyền truy xuất vector chéo doanh nghiệp khi chạy chat AI RAG.
- **Bảng `AuditLogs`:** Bảng lịch sử bất biến (Append-only). Ghi lại mọi hành động (Action Type: CREATE, UPDATE, DELETE) kèm theo ID của người thực hiện và mốc thời gian. Liên kết `N-1` với `Tenants`.(Tên công ty, Domain, Invite Code, Settings).
- **Bảng `Users`:** Chứa thông tin đăng nhập của cá nhân. Liên kết `N-1` với `Tenants`.
- **Bảng `Roles` & `Permissions`:** Quản lý cơ chế RBAC động. Mỗi `Tenant` có thể tự tạo các Role riêng (VD: "IT Helpdesk L1") và gán quyền. Liên kết `1-N` với `Users`.
- **Bảng `Folders`:** Lưu trữ cây thư mục đệ quy. Chứa `parentId` trỏ lại chính nó để tạo cấu trúc lồng nhau không giới hạn. Liên kết `N-1` với `Tenants`.
- **Bảng `Documents`:** Lưu trữ bài viết, nội dung. Chứa `folderId` và `tenantId`. Liên kết `N-1` với `Tenants`.
