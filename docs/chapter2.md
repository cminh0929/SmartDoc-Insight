...
3. Backend xử lý Business Logic và truy vấn xuống Database (PostgreSQL) hoặc Search Engine (Meilisearch) tùy thuộc vào tác vụ.
4. Dữ liệu được trả về dưới dạng JSON và Client sẽ đảm nhiệm việc render lên màn hình.
```mermaid
graph TD
    %% Define Styles
...
### 2.2.3 Database & ORM (Storage Layer): PostgreSQL & Drizzle
- **PostgreSQL:** Cơ sở dữ liệu quan hệ mạnh mẽ, ổn định, hỗ trợ rất tốt cho việc lưu trữ cấu trúc cây (đệ quy thư mục) và tính năng JSONB (hỗ trợ phân quyền động).
